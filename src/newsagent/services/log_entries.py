...
newsagent.logging_setup, the CLI's pipeline-run correlation patch) goes
...
def record_log(
    ...
    *,
    ...
    """Persist one log record. pipeline_run_id starts NULL - a pipeline run's
    own `pipeline_runs` row doesn't exist until the run finishes, so records
    emitted during the run are correlated after the fact via attach_pipeline_run."""
...
def attach_pipeline_run(db: Session, log_entry_ids: list[int], pipeline_run_id: int) -> None:
    """Back-fill pipeline_run_id on records emitted during a run, once that
    run's `pipeline_runs` row - and therefore its id - exists."""
