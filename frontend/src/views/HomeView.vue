    ...
    <template v-else>
      <section class="hero">
        ...
        <p v-if="signInRequired" class="signin-note">
          ...
          <img :src="googleWordmark" alt="Google" class="google-mark" />
          תחילה.
        </p>
...
import googleWordmark from "@/assets/google-wordmark.svg";
...
/* Tailwind's preflight makes every img display:block - inside this sentence
   the wordmark has to sit in the line box like a word. Sized in em so it
   tracks the sentence rather than a fixed pixel height.
   The artwork fills its viewBox: cap height is 78% of it and the 'g'
   descender the remaining 22%, so this height puts the logo's caps at the
   same 10px the word "Google" had as plain text, and the negative
   vertical-align drops its baseline onto the text baseline. */
.google-mark {
