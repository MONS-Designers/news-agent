<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight">Digest engagement</h1>
        <p class="mt-1 text-sm text-neutral-500">
          Opens and link clicks for digests that have been sent.
        </p>
      </div>
      <button
...
async function loadEngagement() {
  ...
  try {
    ...
    if (error instanceof ApiError && error.status === 401) {
      errorMessage.value = "Sign in with Google to view digest engagement.";
    } else if (error instanceof ApiError && error.status === 403) {
      errorMessage.value = "Your account does not have admin access.";
    } else {
      errorMessage.value = "Failed to load digest engagement.";
    }
  } finally {
    loading.value = false;
  }
