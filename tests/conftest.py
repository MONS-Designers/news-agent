...
def telemetry_db():
    ...
    SessionLocal() per write, and the thread-propagation tests deliberately
    write from a ThreadPoolExecutor worker. Plain sqlite:///:memory: hands
    each new connection its own empty in-memory database (SQLAlchemy's
    default pool for it is one connection per thread), so a worker thread's
    write would silently land in a different, tableless DB than the one
...
def _isolate_telemetry_from_real_db(monkeypatch: pytest.MonkeyPatch, telemetry_db) -> None:
    """newsagent.telemetry.sink opens its own SessionLocal for every write
    (like newsagent.logging_setup's DB handler already does), specifically so
    a caller deep inside pipeline/ or suggestions/ never has to plumb a
    Session through just to report a call.
    ...
    not a local dev DB. Without this fixture, *every* test that exercises
    send_chat_completion (which is most of tests/llm, tests/pipeline, and
    tests/suggestions) would attempt a real write against it on every run.
    This fixture is autouse specifically so no individual test file has to
    know that risk exists."""
