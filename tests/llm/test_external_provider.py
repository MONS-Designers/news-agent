...
def test_result_is_unchanged_when_the_backend_reports_no_usage():
    """The provider-reported token counts no longer attach to the result at
    all (that mechanism moved to newsagent.telemetry - see
    tests/telemetry/test_call_recording.py for its coverage); this just
    checks a missing usage block doesn't otherwise change a good result."""
