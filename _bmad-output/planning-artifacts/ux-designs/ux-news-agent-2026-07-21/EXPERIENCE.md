...
- **Resolved (2026-09-09) - contrast measured against WCAG 2.1 AA (4.5:1 normal text):** every ink-scale token against `{colors.bg-void}`, and text on `chip-selected`/`topic-pill picked` gradient fills, computed via the standard relative-luminance formula (script: none checked in, ad hoc). Results:
  - `ink-primary`/`ink-body`/`ink-secondary`/`ink-tertiary` on `bg-void`: 17.95:1 / 17.16:1 / 11.85:1 / 6.31:1 - all pass with margin, no change needed.
  - `ink-muted` and `ink-faint` on `bg-void` **failed** (4.05:1 and 3.04:1 - both used at 11-13px, never large-text-exempt): raised to `#828a9e` (5.62:1) and `#767d92` (4.73:1) respectively, keeping the tier ordering (tertiary > muted > faint) intact. Changed in `style.css`'s `--color-hd-label`/`--color-hd-muted` and mirrored in `DESIGN.md`'s color table - a single shared token, so every consumer (chip/step labels, ghost-button text, progress stepper, placeholders, status captions) picked up the fix automatically.
  - `chip-selected` text (inherited `ink-body`, white-ish) against its gradient fill: 11.9-14.3:1 across the gradient. `topic-pill picked` text (`#fff`) against its gradient fill: 13.5-16.4:1. Both pass with large margin - no change needed.
  - This closes the one real carryover gap from the original prototype era. **GH #31 can now be closed** - see the controls/motion/contrast resolutions in this section.
- **Resolved (2026-09-09) - primary-button gradient text contrast:** `BTN_PRIMARY`'s white button text sits on the `accent-gradient-start`→`accent-gradient-end` gradient, used by every "המשך"/"שמירה" CTA across the wizard, `PreferencesView.vue`, and the `HomeView.vue` login CTA. The original `#7b86ff`→`#5c68e8` measured 3.14:1 at the gradient's top edge and only 4.56:1 at its very bottom edge - at the button's vertical midpoint, where the text actually sits, ~3.77:1, failing WCAG AA's 4.5:1 floor. This went through four rounds before landing - recorded in full since each round's failure mode is a real lesson, not noise:
1. `#5361ff`→`#3947e3` (same hue, same lightness spread as the original, just shifted darker): passed contrast (4.64:1 minimum) but read as visually flat, a pure brightness fade with no sense of depth.
2. `#5460ff`→`#261761` (hue 236°→252°, a ~16° drift toward violet, for a two-axis lightness+hue ramp): the plain `linear-gradient()` default (sRGB interpolation) crosses this hue/lightness distance through a visibly desaturated, muddy-gray band around the midpoint (measured: saturation dips from 100%/62% at the endpoints down to 49% at the middle, non-monotonically - a well-known sRGB-gradient artifact) - read as "unprofessional." Fixed the *mechanism* by interpolating `in oklch` instead (`linear-gradient(to bottom in oklch, ...)` - CSS Color 4 syntax, Chrome 111+/Safari 16.4+/Firefox 128+, already within Tailwind v4's own browser baseline): perceptually uniform, so the same two endpoints produce a smooth, monotonic ramp with no muddy dip (verified by sampling the OKLCH-interpolated gradient at 5% steps: contrast rises smoothly 4.67:1→15.24:1, no low point) - confirmed visually in a live side-by-side before rollout. Every subsequent round keeps this `in oklch` mechanism and only changes the two endpoint colors.
3. Same mechanism, pushed much darker (`#434ed2`→`#261761`-ish territory tested down to near-black, e.g. `#050027`): text contrast was never a problem (only improves as colors darken), but the fill's contrast *against `bg-void`* fell to ~1:1 - at that depth the button's own edges disappear into the page, which reads as broken, not premium. A dark UI on an already near-black canvas has a real floor on how dark a filled control can go before it loses its own silhouette.
4. **Landed:** `#434ed2`→`#231666`, `in oklch`, plus a 1px `rgba(169,177,255,0.3)` border (new - the fill alone can't carry edge definition at this depth, so the border does; reuses the same subtle-border convention already used on chips/panels elsewhere in Hybrid Depth, applied to the primary button for the first time) and a deliberately restrained shadow (`0 4px 12px -6px rgba(109,123,255,0.35)` - an earlier, louder attempt at `0 12px 32px -8px / 0.75` read as an oversized halo, not depth). Text contrast 6.44:1-15.24:1; fill-vs-void 3.01:1 (top) down to 1.27:1 (bottom, carried by the border). Verified in a live rendered comparison (not just computed) before rollout, after three prior rounds each missed on a different visual axis. Changed in all 5 consumers: the 4 Tailwind-utility components (`AboutYouStep.vue`, `InterestsStep.vue`, `TopicsStep.vue`, `PreferencesView.vue`) via Tailwind's arbitrary-property syntax (`[background-image:linear-gradient(to_bottom_in_oklch,_#434ed2,_#231666)] border-[1px] border-[#a9b1ff]/30`, since `bg-gradient-to-b`/`from-*`/`to-*` has no interpolation-space modifier), and `HomeView.vue`'s `.cta` (plain CSS, including scaling its previously-larger hover glow down to match: `0 8px 20px -8px / 0.5`). Mirrored in `DESIGN.md`'s `accent-gradient-start`/`accent-gradient-end`/`accent-gradient-border`.
5. **Explicitly considered and rejected: adding gold.** DESIGN.md's Anti-patterns list already bans a second bright accent color in Hybrid Depth surfaces, naming the brand mark's literal gold specifically - introducing it here would have been a deliberate, undocumented reversal of that rule for one button. User confirmed: stay within the single blue-violet accent family; reach depth/luxury through darkness, contrast, and a border instead.

**Extended (2026-09-09) to the wizard's other selection controls**, same direction (richer fill, no second accent), verified in a rendered comparison before rollout:
- `ChipRow.vue`'s `CHIP_SELECTED` (Field/Role chips): gradient deepened `from-hd-accent-2/22 to-hd-accent-2/10` → `from-hd-accent-2/32 to-hd-accent-2/14`; its glow trimmed `0 8px 24px -8px / .45` → `0 4px 14px -6px / .35` to match the CTA's now-established "no loud halo" rule (the crisp `0 0 0 1px` ring is unchanged).
- `TopicsStep.vue`'s `TOPIC_PICKED` (and its verbatim copy in `PreferencesView.vue`'s read-only `TOPIC_READONLY_PICKED`): same gradient deepening, `22/9` → `32/14`.
- `AboutYouStep.vue`'s Experience (years) segmented control: this was the one true outlier - a flat `bg-hd-accent-2/18` with no border at all, unlike every other selected state in the flow, which is likely why it read as flatter/more "industrial" than its neighbors. Given a border (`border-hd-accent-2/55`, transparent on the unselected state to avoid a layout shift) and the same deepened gradient fill, bringing it in line with the chip/pill pattern instead of inventing a fourth one.
All contrast-checked before rollout (text-on-fill stays 11:1+ throughout, no near-miss) - this round was a richness/consistency pass, not an accessibility fix.
- **New surfaces (summary panel, read-only topic pills, edit-profile button, subscription-toggle row, alert frame, 2026-09-08):** the buttons are real `<button>` elements from day one (this section's controls gap never applied to them), and they inherit the same `prefers-reduced-motion`/`hover:hover` handling by living inside the same `ProfilePickerShell`-managed surface once implemented. They share the same ink-scale tokens and the same primary-button gradient (both now passing, see above) as the wizard.
