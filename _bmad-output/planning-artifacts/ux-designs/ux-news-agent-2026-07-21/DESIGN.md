...
Hybrid Depth is the visual identity for the profile system on the existing, always-editable `/me/preferences` page: the Field / Role / Experience Bucket / Topic picker wizard, the returning-user summary/edit screen it leads to, and the weekly-email subscription toggle beside it (extended 2026-09-08 - originally wizard-only, see `.memlog.md`). It is deliberately not a redesign of the whole app - the rest of the app (including the plain Tailwind Topic toggle list below this picker) keeps its current spartan look, and the daily-digest email keeps its own separate "Midnight" identity. **Revised 2026-09-09:** the page's own top-level heading and its loading/error states are now inside Hybrid Depth too, reversing the earlier carve-out. Leaving them in the spartan baseline put a hard light/dark seam across the middle of `/preferences`: a light heading and a light spinner sitting directly on top of a near-black panel, so the screen visibly jumped from one identity to the other the moment the first fetch resolved. It also stacked two headings in wizard mode, since `ProfilePickerShell` carried its own. `PreferencesView.vue` now owns a single `HybridDepthBackground` wrapping all four states (loading, error, summary, wizard) and the screen's single heading, whose text swaps per state; `ProfilePickerShell.vue` is the stepper and step panel only. The boundary is now the app chrome - the white sticky header and the light page ground around `<main>` - not a line inside the page. This is the first custom visual identity applied to the live app itself, scoped to the profile surfaces it was commissioned for.
