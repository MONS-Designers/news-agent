...
- **Live browser verification was explicitly declined this story** — the user approved the migration but asked to skip standing up the dev servers again. AC #1, #3, #4 (the frontend-observable criteria) are therefore verified by `vue-tsc --noEmit` (clean) and code inspection only, not by an actual browser session. This is a gap relative to Stories 1.1/1.2's verification depth; flagging it rather than claiming equivalent confidence.
...
- AC #1 — native radio group implemented (`<fieldset>` + 4 `<input type="radio" name="experience-bucket">`, visually hidden but focusable, not `display:none`), not the button+`aria-pressed` pattern Field/Role use — confirmed by reading the rendered template and CSS; not confirmed in a live browser this story (see Debug Log).
- AC #2 — `EXPERIENCE_BUCKETS` is a closed set validated server-side; `test_experience_bucket_never_creates_a_suggestion` pins that it never touches `PendingTaxonomySuggestion`. Nothing in the codebase reads `experience_bucket` for suggestion purposes (nothing reads it at all yet — no suggestion computation exists), satisfying the AC vacuously as scoped in Dev Notes.
- AC #3 — `canContinue` extended to a three-way `&&`; `test_each_valid_bucket_saves`/`test_invalid_bucket_is_rejected` cover the service side.
- AC #4 — `ProfilePickerShell.vue`'s panels switched from `v-if`/`v-else-if`/`v-else` to `v-show`, so `AboutYouStep`'s local refs are never destroyed by step navigation; a `watch(currentStep)` replays the entrance animation via the mockup's own reflow trick, independent of the (now removed) destroy/recreate cycle that used to cause replay as a side effect. Not live-verified in a browser this story.
...
- 2026-07-27: Story 1.3 implemented end-to-end — `User.experience_bucket`, closed-set validation in `services/profile.py`, extended `PUT /me/profile`, native radio-group segmented control in `AboutYouStep.vue`, and the `v-show`/animation-replay/Back-navigation fix in `ProfilePickerShell.vue`. 9 new tests (185 total). Migration `c3d4e5f6a7b8` applied to the local dev DB with approval; browser verification explicitly skipped this round at the user's request.
